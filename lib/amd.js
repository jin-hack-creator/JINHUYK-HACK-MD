...
const SuhailMDConnect = require("@whiskeysockets/baileys").default;
const {
  proto,
...
async function syncdb() {
  ...
  _0x5447f8.ev.on("connection.update", async _0x3fd0b3 => {
    ...
    if (_0x2224a7 === "open") {
      ...
      global.allCommands = events.commands;
      let _0x1f88ec = 
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^ASTA_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/JINHUYK;;;/gi, "").replace(/Astro;;;/gi, "").replace(/Astropeda;;;/gi, "").trim();
